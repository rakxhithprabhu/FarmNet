...
def train(dataset_path: str | Path = RECOMMENDATION_DATASET_PATH) -> dict:
    ...
    importance = feature_importance_table(model, list(preprocessor.get_feature_names_out()))
    print(json.dumps(metrics, indent=2))
    print("Feature importance:")
    print(importance.to_string(index=False))

    RECOMMENDATION_MODEL_PATH.parent.mkdir(parents=True, exist_ok=True)
    joblib.dump({"model": model, "preprocessor": preprocessor}, RECOMMENDATION_MODEL_PATH)
    metadata = {
